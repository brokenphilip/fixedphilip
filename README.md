...
fixedphilip is a general-purpose Discord bot written in C++20, utilizing the [D++](https://dpp.dev/) library. The bot aims to provide a convenient set of features for everyday use, primarily ones not found in (or executed better than) other Discord bots. It is designed to support both modern slash-commands (for guild/server and per-user installs), as well as optional old-style (chat prefix) commands (provided the "Message Content" privileged intent is enabled).
