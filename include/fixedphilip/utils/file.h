...
namespace fixedphilip::file
{
	...
	struct base
	{
		...
		// Load data structure from file, based on load settings. On success, returns 's_success' - on failure, returns:
		// - 's_open_file_error' if it failed to open the file for reading (or writing default data)
		// - 's_write_error' if it failed to write defaults to the file
		// - 's_read_error' if it failed to read the file
		// - 's_file_not_found' if the file was not found (default data will be written, if enabled)
		// - 's_parse_error' if the data couldn't be parsed (load_to_struct == false)
		inline status load(const settings& load_settings)
		{
			...
			if (!std::filesystem::exists(filename))
			{
				if (load_settings.create_if_not_found)
				{
					if (log)
					{
						fixedphilip::log::warning(std::format("Error loading '{}' - file not found, creating default...", filename));
					}
					...
					if (log)
					{
						if (result == s_open_file_error)
						{
							fixedphilip::log::error(std::format("Failed to open file '{}' for creating defaults", filename));
						}
						else if (result == s_write_error)
						{
							fixedphilip::log::error(std::format("Failed to write defaults to file '{}'", filename));
						}
					}

					// if successful, default file was created - but still return "file not found"
					return (result == s_success) ? s_file_not_found : result;
				}
		...
		inline status save(const settings& save_settings)
		{
